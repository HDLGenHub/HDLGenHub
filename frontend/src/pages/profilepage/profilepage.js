...
const ProfilePage=()=>{
    ...
    const fetchUser=async()=>{
        if(user.role==='student'){
            const res = await axios.get(`${process.env.SERVER}/Student/${user.data._id}`);
            setUser(res);
            console.log(res);
            ...
            const res = await axios.get(`${process.env.SERVER}/Teacher/${user.data._id}`);
            setUser(res);
            console.log(res);
    ...
    const handleSave=async()=>{
        ...
        try{
            ...
            if(user.role==='student'){
                console.log("Student");
                const res = await axios.put(`${process.env.SERVER}/Student/${id}`,{
                    name,
                    email,
                    registrationnumber,
                    age,
                ...
                console.log("Teacher");
                const res = await axios.put(`${process.env.SERVER}/Teacher/${id}`,{
                    name,
                    email,
                    registrationnumber,
                    age,
        ...
        return(
            <div className='profilecontainer'>
                ...
                <div className='profileform'>
                    <div className='email'>
                        <label>Email</label>
                        <input value={user.data.email} onChange={(e)=>setEmail(e.target.value)}/>
                    </div>
                    <div className='registrationnumber'>
                        <label>registration Number</label>
                        <input value={user.data.registrationnumber} onChange={(e)=>setRegistrationnumber(e.target.value)}/>
                    </div>
                    <div className='age'>
                        <label>Age</label>
                        <input value={user.data.age} onChange={(e)=>setAge(e.target.value)}/>
                    </div>
                    <div className='dpurl'>
                        <label>DP Url</label>
                        <input placeholder={user.data.dp} onChange={(e)=>setDp(e.target.value)}/>
                    </div>
                    <div className='gender'>
                        <label>Gender</label>
                        <input value={user.data.gender} onChange={(e)=>setGender(e.target.value)}/>
                    </div>
                    <div className='chatid'>
                        <label>Chat Id</label>
                        <input value={user.data.chatid} onChange={(e)=>setChatid(e.target.value)}/>
                    </div>
